...
def test_start_consumes_message_batches(mock_kafka_config_loader):
    handler = Mock()
    listener = KafkaListener("test_topic", handler, mock_kafka_config_loader, batch_size=10, poll_timeout=1.0)
    ...
    mock_kafka_config_loader.consumer.consume.side_effect = [
        [mock_msg_success, mock_msg_error],  # one valid message + one errored message
        [],  # nothing queued this round
        Exception("Stop Loop"),
    ]
    ...
    listener.consumer.consume.assert_called_with(num_messages=10, timeout=1.0)
    handler.assert_called_once_with([b'{"data": 1}'])
    listener.consumer.commit.assert_called_once_with(message=mock_msg_success)
    assert listener.consumer.consume.call_count == 3
